...
elegant and most versatile. ggplot2 implements the **grammar of
...
ggplot2 before you start, I'd recommend reading "The Layered
Grammalibraryr of Graphics",
...
`ggplot2::ggplot()` tells you explicitly that we're using the `ggplot()`
...
(aka `ggplot2::mpg`). A data frame is a rectangular collection of
...
colors of your points to the `class` variable to reveal the class of
each car.
...
(If you prefer British English, like Hadley, you can use `colour`
instead of `color`.)
...
unique value of the variable, a process known as **scaling**. ggplot2
...
the transparency of the points, or to the shape aesthetic, which
...
*outside* of `aes()`. You'll need to pick a level that makes sense for
that aesthetic:
...
the difference. Make sure that every `(` is matched with a `)` and every
`"` is paired with another `"`. Sometimes you'll run the code and
nothing happens. Check the left-hand of your console: if it's a `+`, it
means that R doesn't think you've typed a complete expression and it's
waiting for you to finish it. In this case, it's usually easy to start
from scratch again by pressing ESCAPE to abort processing the current
command.

One common problem when creating ggplot2 graphics is to put the `+` in
...
function by running `?function_name` in the console, or selecting the
...
to `ggplot()`. For instance, to make the plots above, you can use this
code:
...
`drv` value, which describes a car's drivetrain. One line describes all
of the points with a `4` value, one line describes all of the points
...
which you can find at <http://rstudio.com/resources/cheatsheets>. To
...
the `linetype` example). It is convenient to rely on this feature
...
wanted to change the y-axis to display `cty` instead of `hwy`. You'd
...
(You'll learn how `filter()` works in the chapter on data
transformations: for now, just know that this command selects only the
subcompact cars.)
...
chart displays the total number of diamonds in the `diamonds` dataset,
grouped by `cut`. The `diamonds` dataset comes in ggplot2 and contains
...
**stat**, short for statistical transformation. The figure below
describes how this process works with `geom_bar()`.
...
can recreate the previous plot using `stat_count()` instead of
`geom_bar()`:
...
1.  You might want to override the default stat. In the code below, I
    ...
    (Don't worry that you haven't seen `<-` or `tribble()` before. You
    might be able to guess at their meaning from the context, and you'll
    learn exactly what they do soon!)
...
so you can get help in the usual way, e.g. `?stat_bin`. To see a
...
colour a bar chart using either the `colour` aesthetic, or, more
usefully, `fill`:
...
or is there one special combination of `hwy` and `displ` that contains
109 values?
...
-   `coord_flip()` switches the x and y axes. This is useful (for
...
-   `coord_quickmap()` sets the aspect ratio correctly for maps. This is
    very important if you're plotting spatial data with ggplot2 (which
    unfortunately we don't have the space to cover in this book).
...
you can use to make *any* type of plot with ggplot2. To see this, let's
